...
function rc_get_content($id) {
	...
	global $wpdb, $comment;
	$comment = $wpdb->get_row($wpdb->prepare("SELECT comment_content, comment_post_ID FROM $wpdb->comments, $wpdb->posts WHERE comment_post_ID = ID AND post_status = 'publish' AND comment_ID = " . $id));
...
function rc_get_avatar_url($code) {
	$code = preg_replace('/^.+(src=)(\"|\')/i', '', $code);
	$code = preg_replace('/(\"|\')\sclass=(\"|\').+$/i', '', $code);
...
function rc_build_comments_json($comments) {
	...
	foreach($comments as $comment) {
		...
		$json .= '"postTitle":"'		. get_the_title($comment->comment_post_ID)							. '",';
